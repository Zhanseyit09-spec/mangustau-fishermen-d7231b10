...
// Simulated AI-detected fish school density per zone (0-1)
const AI_DENSITY: Record<Location, number> = {
  Aktau: 0.72,
  Bautino: 0.94,
  Kuryk: 0.61,
};

export function MangystauMap({ totals, aiMode = false }: Props) {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-lg border border-border/60 bg-gradient-to-br from-[oklch(0.20_0.06_240)] to-[oklch(0.14_0.05_240)]">
      <svg viewBox="0 0 100 100" className="block w-full" preserveAspectRatio="xMidYMid meet">
        <defs>
          ...
          <radialGradient id="heat" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="oklch(0.85 0.22 50)" stopOpacity="0.85" />
            <stop offset="40%" stopColor="oklch(0.72 0.22 25)" stopOpacity="0.5" />
            <stop offset="100%" stopColor="oklch(0.55 0.20 10)" stopOpacity="0" />
          </radialGradient>
        </defs>
        ...
        {/* AI heatmap overlay */}
        {aiMode && LOCATIONS.map((loc) => {
          const c = LOCATION_COORDS[loc];
          const d = AI_DENSITY[loc];
          const r = 10 + d * 14;
          return (
            <g key={`heat-${loc}`}>
              <circle cx={c.x} cy={c.y} r={r} fill="url(#heat)">
                <animate attributeName="r" values={`${r - 3};${r + 3};${r - 3}`} dur="3s" repeatCount="indefinite" />
                <animate attributeName="opacity" values="0.75;1;0.75" dur="3s" repeatCount="indefinite" />
              </circle>
              {/* school dots */}
              {Array.from({ length: Math.round(4 + d * 6) }).map((_, i) => {
                const angle = (i / 8) * Math.PI * 2;
                const dist = 2 + (i % 3) * 2;
                const cx = c.x + Math.cos(angle) * dist;
                const cy = c.y + Math.sin(angle) * dist;
                return (
                  <circle key={i} cx={cx} cy={cy} r="0.6" fill="oklch(0.95 0.15 80)" opacity="0.9">
          ...
          return (
            <g key={loc}>
              ...
              <text x={c.x + 3} y={c.y + 2.5} fontSize="2.6" fill="oklch(0.85 0.05 195)">
                {aiMode ? `${Math.round(AI_DENSITY[loc] * 100)}% шоғыр` : `${totals[loc].toFixed(0)} кг`}
              </text>
            </g>
          );
        })}
      ...
      <div className="absolute bottom-2 right-2 flex items-center gap-3 rounded-md bg-background/70 px-2.5 py-1.5 text-[10px] backdrop-blur">
        {aiMode ? (
          <>
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[oklch(0.85_0.22_50)]" /> Балық шоғыры</span>
